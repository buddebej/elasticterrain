 ...
 * @type {string}
 */
ol.renderer.webgl.tiledemlayer.shader.Fragment.OPTIMIZED_SOURCE = 'precision highp float;uniform sampler2D a;uniform float b;varying vec2 c;float decodeElevation(in vec4 colorChannels){float elevationM=((colorChannels.r*255.0+(colorChannels.g*255.0)*256.0)-11000.0)/10.0;return elevationM;}uniform sampler2D g;uniform sampler2D h;uniform bool i;uniform vec2 j;uniform bool k;uniform bool l;uniform vec3 m;uniform float n;uniform float o;uniform float p;uniform bool q;const float MAX_ELEVATION=9000.0;const highp float CELLSIZE=0.00390625;const highp float RES=0.00390625;void main(void){vec2 m_texCoord=c;float absElevation=decodeElevation(texture2D(a,m_texCoord.xy));vec3 neighbourRight=vec3(m_texCoord.x+CELLSIZE,1.0-m_texCoord.y,0.0);vec3 neighbourAbove=vec3(m_texCoord.x,1.0-m_texCoord.y+CELLSIZE,0.0);neighbourRight.z=decodeElevation(texture2D(a,vec2(m_texCoord.x+CELLSIZE,m_texCoord.y)));neighbourAbove.z=decodeElevation(texture2D(a,vec2(m_texCoord.x,m_texCoord.y-CELLSIZE)));if(m_texCoord.x>=1.0-RES){neighbourRight=vec3(m_texCoord.x-RES,1.0-m_texCoord.y,0.0);neighbourRight.z=decodeElevation(texture2D(a,vec2(m_texCoord.x-RES,m_texCoord.y)));}if(m_texCoord.y<=RES){neighbourAbove=vec3(m_texCoord.x,1.0-(m_texCoord.y+RES),0.0);neighbourAbove.z=decodeElevation(texture2D(a,vec2(m_texCoord.x,m_texCoord.y+RES)));}vec3 currentV=vec3(m_texCoord.x*b,(1.0-m_texCoord.y)*b,absElevation);vec4 fragColor=vec4(0.0,0.0,0.0,0.0);if(i){fragColor=texture2D(h,m_texCoord);}else{float colorMin=o/MAX_ELEVATION;float colorMax=p/MAX_ELEVATION;float relativeElevation=absElevation/MAX_ELEVATION;if(relativeElevation<=colorMin){relativeElevation=0.0;}else if(relativeElevation>=colorMax){relativeElevation=1.0;}else{relativeElevation=(relativeElevation-colorMin)/(colorMax-colorMin);}fragColor=abs(texture2D(g,vec2(0.5,relativeElevation)));if(currentV.z==absElevation&&neighbourRight.z==absElevation&&neighbourAbove.z==absElevation){if(absElevation<=0.0){fragColor=vec4(0.5058823529,0.7725490196,0.8470588235,1.0);}else if(k){fragColor=vec4(0.5058823529,0.7725490196,0.8470588235,1.0);}}}if(l){neighbourRight.xy*=b;neighbourAbove.xy*=b;vec3 normal=normalize(cross(neighbourRight-currentV,neighbourAbove-currentV));if(m_texCoord.x>=1.0-CELLSIZE){normal=normalize(cross(currentV-neighbourRight,neighbourAbove-currentV));}if(m_texCoord.y<=RES){normal=normalize(cross(currentV-neighbourRight,neighbourAbove-currentV));}float hillShade=clamp(n*1.0+max(dot(normal,normalize(m)),0.0),0.0,1.0);vec4 hillShadeC=vec4(hillShade,hillShade,hillShade,1.0);float hillShadeD=clamp(hillShade,0.2,1.0);vec4 hillShadeA=vec4(hillShadeD,hillShadeD,hillShadeD,1.0);float hillShadeL=clamp(hillShade,0.1,0.4);vec4 hillShadeB=vec4(hillShadeL,hillShadeL,hillShadeL,1.0);vec4 zero=vec4(0,0,0,0);vec4 one=vec4(1.0,1.0,1.0,1.0);vec4 two=vec4(2.0,2.0,2.0,2.0);gl_FragColor=hillShadeC*fragColor;}else{gl_FragColor=fragColor;}if(q){float lineWidth=3.0*CELLSIZE;if(m_texCoord.x>=1.0-lineWidth){gl_FragColor=vec4(0.0,0.0,1.0,1.0);}if(m_texCoord.x<=lineWidth){gl_FragColor=vec4(1.0,0.0,0.0,1.0);}if(m_texCoord.y<=lineWidth){gl_FragColor=vec4(0.0,1.0,0.0,1.0);}if(m_texCoord.y>=1.0-lineWidth){gl_FragColor=vec4(0.0,0.5,0.5,1.0);}if(mod(m_texCoord.x,65.0*CELLSIZE)<CELLSIZE){gl_FragColor=vec4(0.9,0.9,0.9,0.1);}if(mod(m_texCoord.y,65.0*CELLSIZE)<CELLSIZE){gl_FragColor=vec4(0.9,0.9,0.9,0.1);}}}';
