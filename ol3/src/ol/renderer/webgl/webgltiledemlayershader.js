 ...
 * @type {string}
 */
ol.renderer.webgl.tiledemlayer.shader.Fragment.OPTIMIZED_SOURCE = 'precision highp float;uniform sampler2D a;uniform float b;uniform float c;uniform float d;varying vec2 e;float decodeElevation(in vec4 colorChannels,in float exaggeration){float elevationM=((colorChannels.r*255.0+(colorChannels.g*255.0)*256.0)-11000.0)*max(exaggeration*10.0,1.0);return elevationM;}uniform sampler2D j;uniform sampler2D k;uniform bool l;uniform bool m;uniform bool n;uniform bool o;uniform vec2 p;uniform vec3 q;uniform float r;uniform float s;uniform float t;uniform float u;const float MAX_ELEVATION=8800.0;const highp float CELLSIZE=0.00390625;void main(void){vec2 m_texCoord=e;float absElevation=decodeElevation(texture2D(a,m_texCoord.xy),0.0);float absElevationEx=decodeElevation(texture2D(a,m_texCoord.xy),s);vec3 neighborRight=vec3(m_texCoord.x+CELLSIZE,1.0-m_texCoord.y,0.0);vec3 neighborLeft=vec3(m_texCoord.x-CELLSIZE,1.0-m_texCoord.y,0.0);vec3 neighborAbove=vec3(m_texCoord.x,1.0-m_texCoord.y+CELLSIZE,0.0);vec3 neighborBelow=vec3(m_texCoord.x,1.0-(m_texCoord.y+CELLSIZE),0.0);neighborRight.z=decodeElevation(texture2D(a,vec2(m_texCoord.x+CELLSIZE,m_texCoord.y)),s);neighborLeft.z=decodeElevation(texture2D(a,vec2(m_texCoord.x-CELLSIZE,m_texCoord.y)),s);neighborAbove.z=decodeElevation(texture2D(a,vec2(m_texCoord.x,m_texCoord.y-CELLSIZE)),s);neighborBelow.z=decodeElevation(texture2D(a,vec2(m_texCoord.x,m_texCoord.y+CELLSIZE)),s);vec3 neighborA=neighborRight;vec3 neighborB=neighborAbove;if(m_texCoord.x>=1.0-CELLSIZE){neighborA=neighborLeft;}if(m_texCoord.y<=CELLSIZE){neighborB=neighborBelow;}vec4 fragColor;if(n){fragColor=texture2D(k,m_texCoord);}else{float elevationRange=d-c;float colorMin=p.x/elevationRange;float colorMax=p.y/elevationRange;float relativeElevation=((absElevation/elevationRange)-colorMin)/(colorMax-colorMin);fragColor=abs(texture2D(j,vec2(0.5,relativeElevation)));float n01=decodeElevation(texture2D(a,vec2(m_texCoord.x+CELLSIZE,m_texCoord.y+CELLSIZE)),s);float n02=decodeElevation(texture2D(a,vec2(m_texCoord.x-CELLSIZE,m_texCoord.y+CELLSIZE)),s);float n03=decodeElevation(texture2D(a,vec2(m_texCoord.x+CELLSIZE,m_texCoord.y-CELLSIZE)),s);float n04=decodeElevation(texture2D(a,vec2(m_texCoord.x-CELLSIZE,m_texCoord.y+CELLSIZE)),s);if(n01==absElevationEx&&n02==absElevationEx&&n03==absElevationEx&&n04==absElevationEx&&neighborRight.z==absElevationEx&&neighborLeft.z==absElevationEx&&neighborAbove.z==absElevationEx&&neighborBelow.z==absElevationEx){vec4 waterBlue=vec4(0.5058823529,0.7725490196,0.8470588235,1.0);if(absElevation<=0.0){fragColor=waterBlue;}else if(l){fragColor=waterBlue;}}}if(m){vec3 currentV=vec3(m_texCoord.x*b,(1.0-m_texCoord.y)*b,absElevationEx);neighborA.xy*=b;neighborB.xy*=b;vec3 normal=normalize(cross(neighborA-currentV,neighborB-currentV));if(m_texCoord.x>=1.0-CELLSIZE){normal=normalize(cross(currentV-neighborA,neighborB-currentV));}if(m_texCoord.y<=CELLSIZE){normal=normalize(cross(currentV-neighborA,neighborB-currentV));}float hillShade=clamp(t*1.0+max(dot(normal,normalize(q)),0.0),0.0,1.0);hillShade=pow(hillShade,1.0/(1.0+r*2.0));hillShade=max(hillShade,0.25);gl_FragColor=vec4(hillShade,hillShade,hillShade,1.0)*fragColor;}else{gl_FragColor=fragColor;}if(o){float criticalEl=c+(d-c)*u;if(absElevation>criticalEl){gl_FragColor=gl_FragColor+vec4(1.0,0.0,0.0,1.0);}if(absElevation<criticalEl){gl_FragColor=gl_FragColor+vec4(0.0,0.5,0.5,1.0);}float lineWidth=2.0*CELLSIZE;if(m_texCoord.x>=1.0-lineWidth){gl_FragColor=vec4(0.0,0.0,1.0,1.0);}if(m_texCoord.x<=lineWidth){gl_FragColor=vec4(1.0,0.0,0.0,1.0);}if(m_texCoord.y<=lineWidth){gl_FragColor=vec4(0.0,1.0,0.0,1.0);}if(m_texCoord.y>=1.0-lineWidth){gl_FragColor=vec4(0.0,0.5,0.5,1.0);}if(mod(m_texCoord.x,65.0*CELLSIZE)<CELLSIZE){gl_FragColor=vec4(0.9,0.9,0.9,0.1);}if(mod(m_texCoord.y,65.0*CELLSIZE)<CELLSIZE){gl_FragColor=vec4(0.9,0.9,0.9,0.1);}}}';
